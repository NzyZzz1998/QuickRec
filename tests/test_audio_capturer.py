...
class TestAudioCapturer(unittest.TestCase):
    ...
    def test_system_track_records_stream_start_time_and_latency(self):
        class FakeRecorder:
            def __enter__(self):
                return self
        ...
        loopback = SimpleNamespace(channels=2, recorder=lambda samplerate: FakeRecorder())
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                capturer.get_track_start_times()[capturer._system_temp_path],
                12.5,
            )
            self.assertEqual(
                capturer.get_track_latency_seconds()[capturer._system_temp_path],
                0.055,
            )
            capturer._cleanup()

    def test_system_output_latency_uses_buffer_and_default_device_period(self):
        player = SimpleNamespace(buffersize=2238, deviceperiod=(0.01, 0.003))
        soundcard = SimpleNamespace(
            default_speaker=lambda: SimpleNamespace(player=lambda samplerate: player),
        )
