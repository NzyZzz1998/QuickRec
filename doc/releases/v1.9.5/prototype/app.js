...
const contracts = {
  ...
  "projects.material-more": contract(
    "项目素材操作",
    "当前项目素材引用已选中。",
    "打开菜单：打开文件、打开目录、在素材库中查看、刷新预览、复制路径和从项目移除。",
    "系统操作或项目引用更新完成后给出明确反馈。",
    "文件缺失时限制打开但保留重新定位；项目保存失败不移除引用。",
    "项目只读时禁用修改类操作，仍允许只读系统操作。",
    "关闭菜单不修改项目。",
    "“从项目移除”只删引用；实际视频、全局素材和预览缓存不变。",
  ),
  "projects.preview-select": contract(
    "选择项目素材",
    "项目详情已加载，列表中存在素材引用。",
    "保持项目上下文，更新右侧静态预览、素材元数据、业务状态和可执行操作。",
    "详情与中央素材事实和当前预览缓存一致；列表滚动位置保持不变。",
    "素材在选择期间变化时刷新为最新状态，不把缓存失败误报为素材失败。",
    "项目不可读取或列表为空时无素材可选。",
    "选择其他素材只改变当前页面状态，不写任何文件。",
    "只读取项目引用、中央素材索引和可再生预览缓存。",
  ),
  "projects.preview-open": contract(
    "打开项目素材",
    "选中素材存在、中央素材记录可用且路径指向普通文件。",
    "调用 Windows 默认播放器打开目标 MP4，项目与选中状态保持。",
    "系统播放器打开正确文件。",
    "文件缺失、没有默认关联或系统调用失败时显示恢复建议。",
    "文件缺失、待关联或路径无效时禁用。",
    "系统操作取消或失败不修改项目、素材或缓存。",
    "不修改任何业务数据。",
  ),
  "projects.preview-folder": contract(
    "打开项目素材所在目录",
    "选中素材存在且文件路径有效。",
    "调用资源管理器并选中目标视频。",
    "资源管理器进入正确目录，项目状态保持。",
  ...
  "projects.preview-library": contract(
    "在素材库中查看",
    "项目引用能解析到中央素材记录。",
    "切换到全局素材库，必要时清除阻止目标显示的临时条件，并定位选中同一 material_id。",
    "显示“来自项目”上下文和返回项目入口；缺失素材可继续重新定位。",
    "中央索引不可用时留在项目页并显示原因。",
    "待关联素材或中央记录不存在时禁用。",
    "返回项目恢复原项目和原素材选中状态。",
    "只改变页面和查询上下文，不修改项目或素材。",
  ),
  "library.return-project": contract(
    "返回来源项目",
    "素材库由项目页跳转进入，且来源项目上下文仍有效。",
    "切换回项目页，恢复来源项目和素材选中状态。",
    "预览详情回到跳转前素材，不重复打开窗口。",
    "来源项目已不可用时进入项目页并显示最新健康状态。",
    "普通素材库访问不显示该入口。",
    "返回只改变页面上下文。",
    "不修改项目、素材或缓存。",
  ),
  "projects.preview-refresh": contract(
    "刷新所选预览",
    "已选择素材且当前没有同素材预览任务。",
    "重新计算文件指纹；可用文件提交高优先级生成任务，缺失文件保留历史预览并提示重新定位。",
    "新 JPEG 校验和原子写入成功后局部更新列表与详情。",
    "FFmpeg、视频、超时或缓存写入失败时保留原业务状态并允许再次刷新。",
    "任务运行中禁用；待关联或缺失素材不读取源文件。",
    "任务开始后可以退出应用取消；已完成缓存保留。",
    "只修改可再生缓存，不写项目文件和中央素材索引。",
  ),
  "projects.preview-rebuild": contract(
    "重建当前项目预览",
    "当前项目可读取，且没有另一个项目预览重建任务。",
    "确认后检查全部素材指纹，只为缺失、失效或明确刷新的预览提交任务；展示成功、失败、跳过和剩余数量。",
    "完成后列表和详情局部更新；重复素材只生成一次有效缓存。",
    "单个素材失败不停止其他任务；缓存目录不可用时给出失败统计。",
    "项目不可读取或重建任务运行中禁用；归档项目仍允许。",
    "取消只取消尚未开始的任务，已完成缓存保留。",
    "不写项目文件、中央项目索引或中央素材索引。",
  ),
...
const contractVersionRules = [
  {
    matches: (key) => key.startsWith("exports.") || key.startsWith("export-config.") || key === "nav.exports" || key === "timeline.export",
    introduced: "v1.9.4",
    optimized: "v1.9.5 保留原导出语义，并补充帧级编辑结果与解绑片段回归边界。",
  },
  {
    matches: (key) => [
      ...
      "timeline.next-frame",
      "timeline.delete-gap",
      "timeline.ripple-menu",
      "timeline.ripple-delete-v195",
      "timeline.unlink",
      "timeline.relink",
      "timeline.editing-fps",
      "timeline.drag-drop",
      "timeline.auto-track",
      "timeline.drag-cancel",
      "timeline.shortcut-focus",
    ].includes(key),
    introduced: "v1.9.5",
    optimized: "本版首次加入；后续版本如改变行为，必须继续在此合同中追加版本记录。",
  },
  {
    matches: (key) => key.startsWith("timeline.") && /(trim|split|ripple|inspector|track-lock|locate-conflict)/.test(key),
    introduced: "v1.9.3",
    optimized: "v1.9.5 统一快捷键、普通删除与波纹删除入口，并按项目编辑帧率提供帧级表达。",
  },
  {
    matches: (key) => key.startsWith("timeline."),
    introduced: "v1.9.2",
    optimized: "v1.9.3 加入基础剪辑；v1.9.4 接入导出；v1.9.5 完善快捷键、关联状态、智能拖放与帧级时间。",
  },
  {
    matches: (key) => key.startsWith("projects.preview") || key === "projects.material-more",
    introduced: "v1.9.1",
    optimized: "v1.9.2 作为时间线素材来源；v1.9.5 拖放时展示吸附、目标轨和自动建轨候选。",
  },
  {
    matches: (key) => key.startsWith("projects.") || key === "nav.projects",
    introduced: "v1.9",
    optimized: "v1.9.1 加入首帧；v1.9.2 接入时间线；v1.9.4 接入导出；v1.9.5 增加项目编辑帧率扩展。",
  },
  {
    matches: (key) => key.startsWith("library.") || key === "nav.library",
    introduced: "v1.6（中央素材索引）",
    optimized: "v1.7 加入搜索/筛选/排序；v1.8 并入工作台；v1.9 起承接项目与时间线素材往返。",
  },
  {
    matches: (key) => key.startsWith("diagnostics.") || key === "nav.diagnostics",
    introduced: "v1.4.1",
    optimized: "v1.8 并入工作台独立页面；v1.9.2 起补充播放/剪辑上下文；v1.9.4 补充导出任务上下文。",
  },
  {
    matches: (key) => key.startsWith("floating."),
    introduced: "v1.0（基础录制浮层）",
    optimized: "v1.8 完成统一视觉与工作台协作；v1.9.5 录制工具栏跟随实际录制屏幕并进入中上安全区。",
  },
  {
    matches: (key) => key.startsWith("settings.") || key === "nav.settings",
    introduced: "v1.0（基础设置）",
    optimized: "v1.4.1 加入诊断路径；v1.8 并入工作台并采用显式保存；v1.9.5 保持焦点优先级，防止误触剪辑快捷键。",
  },
  {
    matches: (key) => key.startsWith("record.") || key === "nav.record" || key === "top.more",
    introduced: "v1.8（现行工作台界面）",
    optimized: "沿用 v1.0 起的录制核心；v1.9 支持项目上下文；v1.9.5 修正录制工具栏目标屏幕定位。",
  },
  {
    matches: (key) => key.startsWith("modal."),
    introduced: "v1.8（统一工作台对话框）",
    optimized: "v1.9 至 v1.9.5 持续补充项目、剪辑和导出事务的取消及失败保护。",
  },
];

const contractShortcutMap = {
  "timeline.play": "Space。仅当剪辑画布拥有操作上下文且焦点不在按钮、输入、菜单或弹窗中时生效。",
  "timeline.previous-frame": "Left；Shift+Left 向前 1 秒。按项目编辑帧率换算。",
  "timeline.next-frame": "Right；Shift+Right 向后 1 秒。按项目编辑帧率换算。",
  "timeline.split": "Ctrl+B。",
  "timeline.delete-gap": "Delete。",
  "timeline.ripple-menu": "Shift+Delete 可直接进入全局波纹影响预览。",
  "timeline.ripple-delete-v195": "Shift+Delete。",
  "timeline.unlink": "Ctrl+L；已关联时解绑，未关联时打开严格候选。",
  "timeline.relink": "Ctrl+L；仅在当前片段未关联时可用。",
  "timeline.undo": "Ctrl+Z。",
  "timeline.redo": "Ctrl+Y。",
  "timeline.trim-cancel": "Esc。",
  "timeline.drag-cancel": "Esc。",
};

function resolveContractVersion(key) {
  const rule = contractVersionRules.find((candidate) => candidate.matches(key));
  return rule || {
    introduced: "v1.8 或更早",
    optimized: "当前原型保留既有语义；后续变更必须在对应正式版本文档中追加记录。",
  };
}

function resolveContractUndo(key, spec) {
  if (spec.undo) return spec.undo;
  if (key.startsWith("timeline.") && /(add|move|trim|split|delete|ripple|unlink|relink|track|drag|editing-fps)/.test(key)) {
    return "成功写入后形成一条离散时间线命令，可用 Ctrl+Z 撤销、Ctrl+Y 重做；保存失败时模型、界面、历史和磁盘一起回滚。";
  }
  if (key.startsWith("projects.") && /(create|rename|archive|restore|add-material|remove-material)/.test(key)) {
    return "没有通用 Ctrl+Z；依赖确认、原子保存和项目备份恢复。失败时保持上一次有效项目。";
  }
  if (key.includes("recycle") || key.includes("delete-confirm")) {
    return "应用内不可撤销；真实文件仅进入 Windows 回收站，恢复由系统回收站承担。";
  }
  if (key.startsWith("settings.save")) {
    return "保存后不提供单步撤销；用户可重新修改并显式保存。保存失败时正式配置不变。";
  }
  if (key.startsWith("exports.")) {
    return "按任务状态提供取消、重试或历史清理；正式输出提交后不由 Ctrl+Z 删除。";
  }
  return "该控件不进入时间线撤销栈；取消、关闭或反向导航按上方规则恢复界面，不隐式回滚已确认的数据写入。";
}
...
function selectContract(element) {
  ...
  if (!spec) {
    console.error("缺少交互契约", key, element);
    return;
  }
  const version = resolveContractVersion(key);
  ...
  document.getElementById("contractIntroduced").textContent = spec.introduced || version.introduced;
  document.getElementById("contractOptimized").textContent = spec.optimized || version.optimized;
  ...
  document.getElementById("contractUndo").textContent = resolveContractUndo(key, spec);
  document.getElementById("contractShortcut").textContent = spec.shortcut || contractShortcutMap[key] || "无固定快捷键；仅通过当前可见控件或系统标准交互触发。";
}
...
function registerContractElement(element) {
  ...
  const version = resolveContractVersion(key);
  element.dataset.versionIntroduced = contracts[key].introduced || version.introduced;
  element.dataset.versionOptimized = contracts[key].optimized || version.optimized;
...
document.querySelectorAll("[data-contract]").forEach((element) => {
  if (contracts[element.dataset.contract]) {
...
function showModal({
  ...
  document.getElementById("modalDialog").classList.toggle("is-wide", wide || actions.length >= 4);
  ...
  actions.forEach((action) => {
    ...
    if (action.contract === "modal.cancel") {
...
function setProjectDisplay(mode) {
  ...
  const previewRebuild = document.getElementById("rebuildProjectPreviews");
  ...
  previewRebuild.disabled = ["missing", "corrupt", "conflict"].includes(mode);
...
const previewAssetByItem = {
  library: "assets/material-library.png",
  diagnostics: "assets/material-diagnostics.png",
  missing: "assets/material-recording.png",
  failed: "",
};

let currentPreviewItem = "library";
let currentPreviewMode = "ready";

function selectedPreviewElement() {
  return document.querySelector(`.project-preview-item[data-preview-item="${currentPreviewItem}"]`)
    || document.querySelector(".project-preview-item");
}

function setPreviewDetail(item, forcedMode = "") {
  if (!item) return;
  currentPreviewItem = item.dataset.previewItem;
  const sourceStatus = item.dataset.status;
  const mode = forcedMode || (sourceStatus === "available" ? "ready" : sourceStatus);
  currentPreviewMode = mode;

  document.querySelectorAll(".project-preview-item").forEach((node) => {
    const selected = node === item;
    node.classList.toggle("is-selected", selected);
    node.setAttribute("aria-selected", selected ? "true" : "false");
  });

  const stage = document.getElementById("projectPreviewStage");
  ...
  const placeholderTitle = document.getElementById("projectPreviewPlaceholderTitle");
  const placeholderText = document.getElementById("projectPreviewPlaceholderText");
  const state = document.getElementById("projectPreviewState");
  const feedback = document.getElementById("projectPreviewFeedback");
  const position = stage.querySelector(".preview-position");
  const openButton = document.getElementById("openProjectMaterial");
  const folderButton = document.getElementById("openProjectMaterialFolder");
  const libraryButton = document.getElementById("locateProjectMaterial");
  const refreshButton = document.getElementById("refreshProjectPreview");

  document.getElementById("previewDetailName").textContent = item.dataset.name;
  document.getElementById("previewDetailPath").textContent = item.dataset.path;
  document.getElementById("previewDetailDuration").textContent = item.dataset.duration;
  document.getElementById("previewDetailFrame").textContent = `${item.dataset.frame} · ${item.dataset.fps}`;
  document.getElementById("previewDetailMode").textContent = item.dataset.mode;
  document.getElementById("previewDetailAudio").textContent = item.dataset.audio;
  document.getElementById("previewDetailSize").textContent = item.dataset.size;
  document.getElementById("previewDetailCache").textContent = mode === "ready" || mode === "missing"
    ? "JPEG · 320×180"
    : "尚无有效缓存";

  stage.className = "project-preview-stage";
  feedback.className = "project-preview-feedback";
  image.hidden = false;
  ...
  state.className = "preview-state-chip badge badge-success";
  state.textContent = "预览可用";
  feedback.querySelector("span").textContent = "预览缓存是可再生数据，不会写入项目文件或中央素材索引。";

  const asset = previewAssetByItem[item.dataset.previewItem];
  ...
  if (mode === "loading") {
    ...
    placeholderTitle.textContent = "正在生成预览";
    placeholderText.textContent = "项目仍可正常操作，完成后会局部更新";
    state.className = "preview-state-chip badge badge-neutral";
    state.textContent = "生成中";
    position.hidden = true;
    refreshButton.disabled = true;
    feedback.querySelector("span").textContent = "后台最多同时执行 2 个任务；单项超时 10 秒并自动重试 1 次。";
  } else if (mode === "failed") {
    image.hidden = true;
    placeholder.hidden = false;
    placeholderTitle.textContent = "无法生成静态预览";
    placeholderText.textContent = "视频不可解析；素材记录和项目引用保持不变";
    state.className = "preview-state-chip badge badge-warning";
    state.textContent = "预览失败";
    position.hidden = true;
    feedback.classList.add("is-warning");
    feedback.querySelector("span").textContent = "可以再次刷新；失败不会被描述为视频保存或素材入库失败。";
  } else if (mode === "missing") {
    stage.classList.add("is-missing");
    state.className = "preview-state-chip badge badge-danger";
    state.textContent = "文件缺失";
    position.textContent = "最后一次有效预览";
    openButton.disabled = true;
    folderButton.disabled = true;
    refreshButton.disabled = true;
    feedback.classList.add("is-danger");
    feedback.querySelector("span").textContent = "文件已移动或删除。历史预览仅用于识别，请前往素材库重新定位。";
  } else {
    position.textContent = "视频 10% 位置";
  }
}

function setPreviewPrototypeState(mode) {
  const target = mode === "missing"
    ? document.querySelector('[data-preview-item="missing"]')
    : mode === "failed"
      ? document.querySelector('[data-preview-item="failed"]')
      : document.querySelector('[data-preview-item="library"]');
  setPreviewDetail(target, mode);
}
...
function applyPrototypeState(state) {
  ...
  if (state === "recording") {
    ...
    routeTo("projects", { force: true });
    const previewState = state.replace("preview-", "");
    if (previewState === "rebuild") {
      setPreviewPrototypeState("ready");
      window.setTimeout(() => document.getElementById("rebuildProjectPreviews").click(), 0);
    } else {
      setPreviewPrototypeState(previewState);
    }
  } else {
...
document.querySelector('[data-contract="projects.more"]').addEventListener("click", () => {
  showModal({
    title: "更多项目操作",
    text: "预览缓存属于可再生数据；系统操作不会修改项目，删除项目始终进入独立安全确认。",
    wide: true,
    actions: [
      ...
      { label: "重建当前项目预览", contract: "projects.preview-rebuild", onClick: () => { closeModal(); document.getElementById("rebuildProjectPreviews").click(); } },
...
document.querySelectorAll(".project-preview-item").forEach((item) => {
  item.addEventListener("click", () => setPreviewDetail(item));
});

document.getElementById("openProjectMaterial").addEventListener("click", () => {
  showToast("已调用默认播放器", selectedPreviewElement().dataset.name, "success");
});

document.getElementById("openProjectMaterialFolder").addEventListener("click", () => {
  showToast("已打开素材目录", selectedPreviewElement().dataset.path);
});

document.getElementById("locateProjectMaterial").addEventListener("click", () => {
  const item = selectedPreviewElement();
  const banner = document.getElementById("projectReturnBanner");
  document.getElementById("projectReturnName").textContent = document.getElementById("projectName").textContent;
  banner.querySelector("small").textContent = `已定位并选中“${item.dataset.name}”`;
  banner.hidden = false;
  routeTo("library", { force: true });
  showToast("已在素材库中定位", item.dataset.name, "success");
});

document.getElementById("returnToProject").addEventListener("click", () => {
  document.getElementById("projectReturnBanner").hidden = true;
  routeTo("projects", { force: true });
  setPreviewDetail(selectedPreviewElement(), currentPreviewMode);
});

document.getElementById("refreshProjectPreview").addEventListener("click", () => {
  const item = selectedPreviewElement();
  if (item.dataset.status === "missing") {
    showToast("无法刷新预览", "文件已缺失，请先在素材库中重新定位。", "warning");
    return;
  }
  setPreviewDetail(item, "loading");
  window.setTimeout(() => {
    if (item.dataset.status === "failed") {
      setPreviewDetail(item, "failed");
      showToast("预览仍无法生成", "视频不可解析；素材记录和项目引用均未改变。", "warning");
      return;
    }
    setPreviewDetail(item, "ready");
    showToast("预览已更新", "新的 JPEG 缓存已校验并原子替换。", "success");
  }, 900);
});

function showPreviewRebuildDialog() {
  showModal({
    title: "重建当前项目预览？",
    text: "系统将检查 8 条项目素材。只重建缺失或已失效的缓存，不修改 project.qrproj、中央项目索引或视频。",
    detail: "预计：有效 6 条，文件缺失 1 条，预览失败待重试 1 条。归档项目也允许执行。",
    icon: "warning",
    actions: [
      { label: "取消", contract: "modal.cancel", onClick: closeModal },
      {
        label: "开始重建",
        kind: "button-primary",
        contract: "projects.preview-rebuild",
        onClick: () => {
          showModal({
            title: "正在重建项目预览",
            text: "后台最多同时执行 2 个 FFmpeg 任务。可以取消尚未开始的任务，已经完成的缓存会保留。",
            detailHtml: `
              <div class="preview-rebuild-progress">
                <div class="preview-progress-track"><span id="previewProgressFill"></span></div>
            ...
            actions: [
              {
                ...
                onClick: () => {
                  closeModal();
                  showToast("已取消剩余任务", "已经完成的 4 份缓存仍然保留。");
                },
              },
            ],
            afterOpen: () => {
              window.setTimeout(() => {
                ...
                window.setTimeout(() => {
                  if (modalLayer.hidden) return;
                  closeModal();
                  showToast("项目预览重建完成", "成功 6，失败 1，跳过 1。单项失败未影响其他素材。", "success");
                }, 600);
              }, 950);
            },
          });
...
document.getElementById("rebuildProjectPreviews").addEventListener("click", showPreviewRebuildDialog);

document.querySelectorAll('[data-contract="projects.material-more"]').forEach((button) => {
  button.addEventListener("click", () => {
    const item = selectedPreviewElement();
    const missing = item.dataset.status === "missing";
    showModal({
      title: "项目素材操作",
      text: missing
        ? "文件已缺失。历史预览只用于识别，可前往素材库重新定位或只从项目移除引用。"
        : "素材仍由全局素材库管理；项目只保存引用，预览缓存可以随时重建。",
      actions: [
        { label: "关闭", contract: "modal.cancel", onClick: closeModal },
        {
          label: missing ? "在素材库中重新定位" : "打开文件",
          contract: missing ? "projects.preview-library" : "projects.preview-open",
          onClick: () => {
            closeModal();
            if (missing) {
              document.getElementById("locateProjectMaterial").click();
            } else {
              document.getElementById("openProjectMaterial").click();
            }
          },
        },
        { label: "刷新预览", contract: "projects.preview-refresh", onClick: () => { closeModal(); document.getElementById("refreshProjectPreview").click(); } },
        {
          label: "从项目移除",
...
document.getElementById("toggleAnnotations").addEventListener("click", (event) => {
  ...
  const annotationsVisible = workbenchBody.classList.contains("show-contracts");
  document.body.classList.toggle("timeline-contracts-open", annotationsVisible);
  event.currentTarget.classList.toggle("is-active", annotationsVisible);
  if (annotationsVisible) {
    const first = document.body.classList.contains("timeline-window-open")
      ? document.querySelector('.timeline-workspace [data-contract]')
      : document.querySelector('[data-contract="record.fullscreen"]');
...
document.getElementById("closeInspector").addEventListener("click", () => {
  ...
  document.body.classList.remove("timeline-contracts-open");
...
setPreviewDetail(document.querySelector('[data-preview-item="library"]'));
