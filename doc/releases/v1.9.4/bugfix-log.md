...
### BUG-194-09 GitHub Windows Runner 发布门禁环境差异

| 项目 | 内容 |
| --- | --- |
| 发现阶段 | RC8 发布提交推送至 `test` 后的 GitHub Actions |
| 严重度 | 发布流程阻塞 |
| 影响 | 同一项目目录的 8.3 短路径和长路径表示导致两项配置测试误判；Packaging job 在 FFmpeg 准备前执行真实 filter graph 测试，导致媒体工具路径为空 |
| 根因 | 配置测试按未经规范化的路径文本构造期望值；CI 只约束 FFmpeg 早于 PyInstaller 构建，没有约束其早于 packaging 集成测试 |
| 失败证据 | GitHub Actions run `30530911164`：配置测试 2 项失败，Packaging 测试 1 项失败 |
| 修复 | 配置测试按产品实现合同比较规范化项目目录；Packaging job 先准备并验证 FFmpeg/FFprobe，再运行 packaging 测试；新增顺序回归测试；运行时代码和 RC8 候选包保持不变 |
| 自动回归 | 标准测试 1238 项通过、32 项排除、66 个子测试通过，覆盖率 84.09%；Packaging 20 项通过；Ruff、Mypy、Compileall 和增量覆盖门禁通过 |
| 状态 | 已关闭；等待同提交远端 CI 复核后进入 master |
...
| RC8 | 正式发布基线 | 已关闭排队目标冲突，通过真实 Windows 125%/150% 导出配置及全部 D11 验收 |

## 5. 当前结论

已确认的九项候选包与发布门禁缺陷均已关闭。RC8 已完成排队目标安全后缀、
150% Qt 等效缩放、真实 Windows 125%/150% 复验和 D11 全部 27 项验收；
RC7 的真实停滞与系统重启恢复、RC6 的目标保护、候选清理、入库重试、
覆盖和取消、RC5 的录制入口互斥以及 RC4 的导出长样本继续作为未受后续
修复影响的继承证据。`LIMIT-194-01` 按已确认产品决策保留为非阻塞已知限制，
v1.9.4 可以进入正式发布流程。
