...
- 严重度：发布阻塞。
- 发现阶段：D11 最终音画同步验收。
- 当前状态：已完成根因定位、最小修复、自动回归、重新打包和定向复验，发布阻塞已关闭。
- 影响范围：1080p120 全屏录制的系统声音及双音频起始时间轴对齐。
...
1. 音频捕获早于视频首帧写入启动，但 WAV 文件不携带捕获单调时钟，最终封装会把
   音频和视频都视为从 0 开始，形成固定起始偏移。
2. 系统回环音频的可感知起点还包含输出缓冲和默认设备周期。当前设备实测为
   `46.625 ms + 10 ms = 56.625 ms`。只按线程启动差补偿会过度裁剪；`r12`
   的复验结果证明这一点。
3. 原实现没有保存每条音轨的启动时刻、设备延迟和视频首帧时刻，混音前也没有
   独立的时间轴对齐步骤。

### 最小修复

- 音频捕获器记录每条音轨的单调时钟启动时间和估算设备延迟。
- 视频编码器在首帧成功写入时记录视频时间轴起点。
- 混音前按每条音轨的有效起点独立执行 `atrim` 或 `adelay`，再进入原有混音链路。
- 系统声音延迟由实际输出缓冲和设备周期计算；麦克风不附加输出延迟。
- 缺少时序数据时保持原有封装行为，不写入硬编码的固定补偿值。
- 日志记录原始起点、设备延迟、有效起点、偏移和采用的对齐操作。

### 自动验证

- 音频捕获与录制管理专项：`63 passed`。
...
- 新增覆盖：启动时间记录、系统输出延迟计算、提前音轨裁剪、延后音轨补时、
  双音轨独立对齐、中文与空格路径及无时序信息兼容。

### 候选包与定向复验

`r12` 只按线程启动差裁剪，出现约 32～65 ms 的音频提前，证明不能忽略系统输出
缓冲；该包已失效，仅保留为排查证据。
...
录制文件: E:\QRtest\QuickRec-v1.8-audio-sync-r13\recordings\QuickRec_20260726_130946.mp4
```

同一时间戳同步源的四次事件偏移为 `+15.104 ms`、`+6.771 ms`、
`+6.771 ms`、`-18.229 ms`，最大绝对偏移为 `18.229 ms`，通过
`40 ms` 发布门槛。原 10 分钟测试的漂移增量为 `-8.333 ms`；本次修复只校正
固定起始时间轴，不改变音频采样或视频帧率节奏，因此长时漂移证据继续有效。
