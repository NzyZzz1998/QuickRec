...
> 当前状态：GUI 手动验收已通过（2026-07-17），可进入发布收口
>
...
- 窗口录制自动化补证限制：两次由桌面自动化选择目标后，目标 HWND 在正式开始前消失，日志记录“目标窗口已不存在，取消窗口录制”，未生成错误文件。该工具限制随后由用户使用稳定窗口人工复测关闭，不判为产品缺陷。
...
| ------------ | --- | ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| 窗口录制         | 通过  | 用户人工确认（2026-07-17）                                                                    | 使用稳定目标窗口完成录制、播放和素材库核对；未虚构额外截图或文件路径。                                                       |
...
| 麦克风          | 通过  | 用户人工确认（2026-07-17）                                                                    | 麦克风单独模式录制与听感复测通过；未虚构额外 MP4 路径。                                                                |
...
| 125% DPI     | 通过  | 用户人工确认（2026-07-17）                                                                    | 素材库及确认框无关键裁切、重叠或不可操作控件。                                                                        |
| 150% DPI     | 通过  | 用户人工确认（2026-07-17）                                                                    | 素材库及确认框无关键裁切、重叠或不可操作控件。                                                                        |
| 正常退出与环境恢复    | 通过  | 用户人工确认（2026-07-17）；`evidence\auto3\verification-summary.json`                         | QuickRec 进程已停止，FPS 恢复为 30，真实 APPDATA 哈希未变化，Lite 工作区干净。                                        |

人工补证已全部完成。自动化证据引用实际文件路径；仅由用户现场确认的项目明确标注为“用户人工确认”，不补写不存在的证据路径。
...
最终结论：**通过，可进入发布收口**。当前没有未关闭的发布阻塞项；正式推送、tag 和 GitHub Release 仍需单独授权。
