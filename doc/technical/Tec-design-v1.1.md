...
**技术选型**：使用 `soundcard`（WASAPI loopback via MediaFoundation）捕获系统声音，`pyaudio` 捕获麦克风。

> **设计决策**：系统声音最初选用 `pyaudiowpatch`，但其 WASAPI loopback 的 `stream.read()` 会无限阻塞（进程挂起，无异常），最终替换为 `soundcard`——其 `recorder.record(numframes=N)` 非阻塞、准确返回 N 帧 float32 numpy 数组。详见 bugfix-log Bug #24。

**核心类**：

```python
class AudioSource:
    """音频源常量（普通类，非 Enum；值即配置字符串）"""
    NONE = "none"               # 无音频（v1.0 默认行为）
    SYSTEM = "system"            # 系统声音（WASAPI loopback）
...
class AudioCapturer:
    ...
    优雅降级：若音频初始化失败，start() 返回 False，继续无声录制，不影响视频。
    """

    - _source: str                       # 音频源类型（AudioSource 字符串值）
    - _sample_rate: int                  # 实际采样率（默认 48000）
    - _channels: int                      # 声道数（系统声音=2ch，麦克风=1ch）
    - _sample_width: int                 # 采样位深（字节），固定 2（16bit）
    - _is_recording: threading.Event     # 录制进行中标志
    - _audio_thread: threading.Thread    # 音频捕获线程
    - _sys_recorder                       # soundcard loopback recorder（系统声音）
    ...
    - _pa_mic: pyaudio.PyAudio          # 麦克风 PyAudio 实例

    + __init__(source: str, output_dir: str)
    + start(output_stem: str = "") -> bool  # 初始化音频流并开始捕获
...
- SYSTEM 模式：一个 WAV 文件（`<stem>.audio_sys.wav`）
- MICROPHONE 模式：一个 WAV 文件（`<stem>.audio_mic.wav`）
- BOTH 模式：两个独立 WAV 文件
...
def _capture_loop(self):
    while self._is_recording.is_set():
        # 系统声音: soundcard.record() 返回 float32 numpy → 转 int16
...
**WASAPI loopback 设备发现**（soundcard）：

```python
def _find_loopback_mic(self):
    """查找默认扬声器对应的 WASAPI loopback 麦克风

    1. 取 sc.default_speaker() 名称
    2. 在 sc.all_microphones(include_loopback=True) 中优先匹配同名 loopback
    3. fallback: 取第一个 isloopback 设备
    若未找到, 返回 (None, 0)，_start_system() 返回 False（降级无声）。
    """
```
...
```python
def show_notification_with_action(self, title, msg,
                                  action_label="打开文件夹", output_path=""):
    """显示带操作按钮的 Toast 通知

    降级链：winotify → pystray.notify()（纯文本）
    """
    ...
    try:
        ...
        toast = Notification(
            app_id="QuickRec",
            title=title,
            msg=msg,          # 注意：winotify 1.1 参数名是 msg，不是 body
        )
        if output_path:
            # winotify action 走 protocol 激活，launch 必须是 URI；
            # 用 file:/// 目录 URI 打开视频所在文件夹（无法选中具体文件）
...
> **关键约束**（见 bugfix-log Bug #27）：
> - winotify 1.1 构造参数是 `msg=` 不是 `body=`（用错会抛 TypeError 静默降级）
> - `add_actions(label, launch)` 是两个独立参数，不能传 list
> - launch 必须是合法 URI（protocol 激活），`explorer.exe /select,...` 命令行无效，故 Toast 按钮只能"打开目录"无法"选中文件"；选中文件由结果条"📂 打开"按钮承担
...
def _start(self, region=None) -> bool:
    ...
    # v1.1 新增: 音频初始化（audio_source 是字符串，直接赋值）
    ...
    if self._audio_source != AudioSource.NONE:
        ...
        if not self._audio_capturer.start(output_stem=...):
...
def _get_ffmpeg_path() -> str:
    ...
    搜索顺序:
    1. 打包后(frozen): sys._MEIPASS/ffmpeg/ffmpeg.exe（PyInstaller 资源目录，
       onedir 模式下指向 _internal），再 fallback 到 sys.executable 同级目录
    2. 开发环境: 项目根目录 ffmpeg/ffmpeg.exe
    3. 系统 PATH 环境变量
    4. 返回空字符串（无音频混合能力，降级保留纯视频）
    """
    # 1. 打包后：优先 _MEIPASS（onedir 下为 _internal），再 sys.executable 同级
...
> **打包注意**：PyInstaller 6.x onedir 模式下，`datas` 声明的文件落入 `_internal/` 子目录而非 exe 同级，因此必须用 `sys._MEIPASS` 定位。详见 bugfix-log Bug #28。
...
| 库 | 版本 | 用途 | 打包影响 |
|----|------|------|---------|
| soundcard | 0.4.6 | WASAPI loopback 系统声音捕获 | ~1MB |
| pyaudio | 0.2.14 | 麦克风音频捕获 | ~2MB（含 PortAudio DLL） |
| winotify | 1.1 | Windows 10/11 Toast 通知 | ~100KB |
| FFmpeg (binary) | 8.0.1 | AAC 音频编码 + 音视频混合 | ~95MB |

**FFmpeg 打包方式**：

- 将 `ffmpeg.exe` 放在项目 `ffmpeg/` 子目录下
- `build_std.spec` 中通过 **`datas=[('ffmpeg/ffmpeg.exe', 'ffmpeg')]`** 打包（PyInstaller 6.x 落入 `_internal/ffmpeg/`）
- `_get_ffmpeg_path()` 按优先级搜索：`sys._MEIPASS` → exe 同级 → 项目目录 → 系统 PATH → 返回空字符串
...
└── recorder/recorder_manager.py（录制模式 + 音频 + FFmpeg）
    ...
    ├── recorder/audio_capturer.py（新增: soundcard + pyaudio）
...
| soundcard loopback 设备兼容性 | 部分设备无 WASAPI loopback | 捕获失败时自动降级为无声录制 |
| WASAPI loopback 设备枚举 | 某些声卡无 loopback 设备 | 自动降级为无声录制；提示用户启用立体声混音 |
| BOTH 模式两路音频采样率不同 | 合并后音频速度异常 | FFmpeg amerge 滤镜自动重采样 |
| FFmpeg 打包体积 | 安装包增加 ~95MB | 可接受；或提供 FFmpeg-free 版本 |
| FFmpeg 不可用 | 无法混入音频 | 降级：保留无声 MP4，通知用户 |
| pystray 动态菜单不刷新 | 录制状态切换后菜单不变 | 使用 MenuItem callable 参数；或 icon.update_menu() |
| winotify 不可用 | Toast 通知失败 | 降级链：winotify → pystray.notify()（纯文本） |