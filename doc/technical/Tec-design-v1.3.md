...
| 编号 | 功能 | 说明 | PRD 编号 |
|-----|------|------|---------|
| E1 | H.264 实时编码 ✅ | 零拷贝管线：dxcam 帧直接通过 FFmpeg stdin pipe 编码 H.264 MP4；libx264 CRF=23 preset=superfast + tune=zerolatency；stderr=DEVNULL 防死锁；无音频时停止即完成 | E1 |
| N1 | 指定窗口录制 **（从延期恢复）** ✅ | 删除 v1.2 遗留注释代码，从零重写；窗口丢失处理：关闭→自停保存+托盘通知，最小化→**录制线程同步阻塞** +托盘通知（无 QMessageBox）；主线程异步化置前台避免闪退 | F3 |
...
| N4 | 打包体积优化 | PyInstaller spec 排除不需要的 Qt 模块/DLL/PIL 编解码器/Qt 插件 + 过滤 ANGLE/OpenCV videoio ffmpeg；打包 334→259MB（核心依赖 ≈215MB 不可压缩，目标 150MB 因 cv2+ffmpeg 体积未达成） | S5 |
| N5 | DPI 缩放适配 | QApplication 高 DPI 属性设置，修复 4K/150% 缩放下 UI 错位 | R1 |

**关键设计决策**：

1. 编码管线：**FFmpeg pipe 实时编码**，dxcam BGR24 帧直接通过 stdin 送 FFmpeg 编码 H.264（CRF 23, preset **superfast** + tune zerolatency）；无音频时停止后文件 < 1 秒可用
2. **stderr 处理关键修正**：FFmpeg `stderr=subprocess.DEVNULL`（不用 PIPE），否则 FFmpeg 进度日志会填满 OS pipe 缓冲（≈64KB）导致进程死锁（v1.3 Bug #58 实测问题）
3. 有音频时：**方案 1（后混合）**——视频仅 pipe 到 FFmpeg 编码（session_dir/video.mp4），音频单独录制 WAV，停止后 FFmpeg 二次转封装混合（-c:v copy -c:a aac，1-3 秒）
4. 窗口录制：**从零重写**（删除 v1.2 所有注释代码），窗口丢失处理简化为"关闭→自停保存，最小化→暂停+托盘通知"；最小化时**录制线程同步阻塞**（`clear(_resume_event)` 后 wait），用户恢复后点"继续" resume 唤醒
5. 临时文件：**会话目录隔离**，录制中视频/音频临时文件统一在 %TEMP%/QuickRec/session_<pid>_<ts>/ 下管理
6. H.264 参数：**内部固定 CRF 23 + preset superfast + tune zerolatency**，不暴露给用户；lookahead=0 降低停止 flush 延迟
7. 磁盘预警：**仅录制前检查一次**，不持续轮询；< 1GB 预警可选继续，< 200MB 阻断
8. 窗口置前台：Alt 键模拟释放 Windows 前台锁 + `SetForegroundWindow/BringWindowToTop`；`_on_window_selected` 主线程异步化（QTimer.singleShot 400ms）避免长阻塞闪退
...
class VideoEncoder:
    ...
    参数：
        ...
        crf: H.264 CRF 质量参数（固定 23）
        preset: H.264 编码预设（固定 "superfast"，lookahead=0 降低停止 flush 延迟）
    """
    ...
    + __init__(output_path, fps, frame_size, ffmpeg_path, crf=23, preset="superfast")
...
def _build_cmd(self):
    return [
        ...
        "-i", "pipe:0",                                         # stdin
        "-c:v", "libx264",
        "-crf", str(self._crf),                                 # 固定 23
        "-preset", self._preset,                                # 固定 superfast
        "-tune", "zerolatency",                                # 零延迟缓冲
...
self._proc = subprocess.Popen(
    ...
    stderr=subprocess.DEVNULL,  # 关键：不用 PIPE；FFmpeg 进度日志会填满 OS 缓冲导致死锁
)
```
...
| `-crf 23` | 质量系数 | 0=无损，23=默认高质量，28=一般；录屏场景文件略大于 medium |
| `-preset superfast` | 编码速度 | lookahead=0，停止时 flush 近乎瞬时（比 medium 显著更快） |
| `-tune zerolatency` | 调优 | 进一步消除延迟缓冲 |
| `-pix_fmt yuv420p` | 输出像素格式 | 确保在所有播放器上正常显示 |
| `stderr=DEVNULL` | 进程 stderr | **关键**：丢弃 FFmpeg 进度日志，避免 pipe 缓冲满（≈64KB）导致 FFmpeg 阻塞写 stderr 死锁 |
...
def close(self) -> bool:
    ...
    2. 等待进程退出（最多 15 秒超时；superfast+zerolatency 实测 < 2 秒）
    3. 检查 returncode
    ...
    try:
        ...
        self._proc.kill()
        self._proc.wait()
        logger.error("FFmpeg 编码超时，已强制终止（stderr 已丢弃无法诊断）")
        return False
...
| stderr pipe 缓冲满 → FFmpeg 死锁 | **关键修正**：`stderr=subprocess.DEVNULL` 丢弃进度日志避免死锁（Bug #58） |
| 进程中途退出（BrokenPipeError） | write_frame 返回 False，recorder_manager 停止录制 |
| close 超时（TimeoutExpired） | 强制 kill()，返回 False；超时阈值 15 秒 |
...
def _record_loop(self):
    ...
    while not self._stop_event.is_set():
        ...
        # v1.3: 窗口模式下定期更新捕获区域（100ms，与 highlighter 同步）
            ...
            if now - last_window_update >= 0.1:
                ...
                if rect is None:
                    ...
                    if not user32.IsWindow(self._window_hwnd):
                        ...
                        reason = "closed"  # 句柄在但 rect 无效 → 视为关闭
                    if not self._window_lost_emitted:
                        self._window_lost_bridge.window_lost.emit(reason)
                        self._window_lost_emitted = True
                    if reason == "closed":
                        break
                    # minimized：录制线程自己同步暂停避免异步 emit 不及时的空转
                    # 清 _resume_event 使上层 wait 阻塞，直到用户点"继续" resume() 唤醒
...
def _stop_and_encode(self):
    """后台线程：等待录制结束，启动最终化"""
    # 关键修正（Bug #50）：join 不设 timeout，必须等录制线程完成 encoder.close()
    # 才能开始 _finalize shutil.move（否则移动了 FFmpeg 仍在写的文件）
...
def start_window(self, hwnd: int) -> bool:
    ...
    关键修正（Bug #57）：v1.3 移除主线程重试循环（避免阻塞 Qt 主线程引发闪退），
    rect 一次性获取失败立即返回 False；窗口恢复/可见性统一由 main._on_window_selected
    异步处理（QTimer.singleShot），不在主线程同步阻塞。
    """
    user32 = ctypes.windll.user32
...
def get_window_hwnd(self) -> int:
    """新增：暴露窗口句柄供 main 重建 highlighter"""
...
def _update_position(self):
    # 使用客户区坐标（GetClientRect + ClientToScreen），与 recorder_manager 同坐标系
    user32 = ctypes.windll.user32
...
**关键修正**：v1.3 实际改用 `GetClientRect + ClientToScreen` 而非 `GetWindowRect`——与 `recorder_manager._get_window_rect` 使用同一客户区坐标系，避免边框覆盖范围（含阴影）与录制内容范围（不含阴影）错位。最小化窗口（`IsIconic`）时隐藏高亮，不画到 (0,0)。

---

### 2.6 磁盘空间预警模块 (disk_checker.py) — 扩展

**职责**：在现有 `DiskChecker` 基础上新增预警/阻断阈值常量和 `check_before_recording()` 方法。新增 `show_disk_warning()` 函数（同模块内，非单独文件）。

**新增常量（模块级，非 DiskChecker 类属性）**：
...
def check_before_recording(save_path: str) -> tuple:
    ...
    free_mb = free_bytes // BYTES_PER_MB   # 模块级常量 BYTES_PER_MB = 1024*1024
...
**预警对话框（同模块内函数 show_disk_warning）**：
...
├── utils/disk_checker.py（扩展: 预警/阻断阈值 + check_before_recording + show_disk_warning 同模块）
...
## 6. 开发里程碑 ✅ 已完成（2026-06-22）

| 阶段 | 内容 | 状态 |
|-----|------|------|
| 1 | video_encoder.py 重写 (FFmpeg pipe) | ✅ `054b57c` |
| 2 | temp_cleaner.py 新增 | ✅ `054b57c` |
| 3 | disk_checker.py 扩展（同模块 show_disk_warning） | ✅ `054b57c` |
| 4 | window_selector.py + window_highlighter.py 重写 | ✅ `e19991b` |
| 5 | recorder_manager.py 重构 (pipe + 会话目录 + 窗口恢复) | ✅ `f145b31` |
| 6 | main.py 窗口录制恢复 + DPI + 磁盘检查 | ✅ `a89e353` |
| 7 | settings_dialog.py + tray_icon.py 小改 | ✅ `a89e353` |
| 8 | build_std.spec 优化（334→259MB） | ✅ `0866808` |
| 9 | 集成测试 + 打包（Bug #50-#58 迭代修复）+ 单测 test_v1_3.py | ✅ 21 通过 / 2 跳过 |
...
| FFmpeg stderr 缓冲满死锁（Bug #58） | 进程卡死、停止 wait 超时被 kill | ✅ `stderr=subprocess.DEVNULL` 丢弃进度日志，避免 64KB 缓冲满阻塞 |
| H.264 实时编码 CPU 占用高于 JPEG | 低端设备录制卡顿 | CRF 23 + preset superfast + tune zerolatency 平衡；lookahead=0 降低 flush 延迟 |
| 窗口录制代码从零重写 | 可能引入新的边界 bug | 参考 v1.2 注释代码中已验证正确的部分（GetClientRect 坐标计算、窗口过滤规则） |
| 特殊窗口（游戏/UWP）置前台失败 | SetForegroundWindow 被前台锁拒绝 | ✅ Alt 键模拟 + BringWindowToTop；主线程异步化避免长阻塞闪退 |
| 窗口丢失给托盘通知 | 托盘通知可能被用户忽略 | 通知文本明确写明当前状态和下一步操作；工具栏暂停状态同步显示 |
| 打包后排除的 DLL 导致启动崩溃 | 关键 Qt 插件被误删 | 保留核心 Qt 模块（Core/Gui/Widgets/Network/WinExtras）；platform/qwindows/qoffscreen 插件必保留 |
| 打包体积不达 150MB 目标 | 核心依赖 cv2+ffmpeg+numpy+Qt ≈215MB 不可压缩 | ✅ 实际 259MB；记录到 PRD 为已知限制 |
