...
## 前置依赖
- [ ] config.py（新增配置项 + get_native_resolution）
...
- [ ] `_spin_countdown_seconds: QSpinBox` — 倒计时秒数输入框
  - 范围 1-10，默认 3
...
### 2. 画质下拉框动态显示
- [ ] `_refresh_quality_combo()` 方法
  - 调用 `ConfigManager.get_native_resolution()` 获取主显示器分辨率
  - "原生"选项显示为 `"原生(1920×1080)"` 格式（动态）
  - 其他选项保持固定文本："高(1080p)"、"中(720p)"、"低(480p)"
- [ ] 在 `__init__` 中调用 `_refresh_quality_combo()`

### 3. 布局调整
- [ ] 将开机自启、录制倒计时、鼠标点击高亮放在同一行（或两行）
- [ ] 窗口录制快捷键放在暂停快捷键之后
- [ ] 保持与 v1.1 设置界面风格一致

### 4. _load_config 扩展
- [ ] 读取 `auto_start` → 设置 `_cb_auto_start` 勾选状态
  - 额外检查注册表实际值：`autostart.is_autostart_enabled()` 确保一致性
- [ ] 读取 `show_countdown` → 设置 `_cb_countdown` 勾选状态
- [ ] 读取 `countdown_seconds` → 设置 `_spin_countdown_seconds` 值
- [ ] 读取 `mouse_highlight` → 设置 `_cb_mouse_highlight` 勾选状态
- [ ] 读取 `shortcut_window` → 设置 `_shortcut_window` 值

### 5. _save_config 扩展
- [ ] 保存 `show_countdown` 到 config
- [ ] 保存 `countdown_seconds` 到 config
- [ ] 保存 `mouse_highlight` 到 config
- [ ] 保存 `auto_start` 到 config
  - 同时调用 `autostart.enable_autostart()` 或 `autostart.disable_autostart()` 操作注册表
- [ ] 保存 `shortcut_window` 到 config

## 验收标准
- [ ] 开机自启勾选后注册表有 QuickRec 项
- [ ] 取消勾选后注册表项被删除
- [ ] 打开设置时开机自启状态与注册表实际值一致
- [ ] 原生画质显示当前实际分辨率（如 "原生(1920×1080)"）
- [ ] 倒计时复选框控制秒数输入框启用/禁用
- [ ] 窗口录制快捷键可录制和保存
- [ ] 所有意控件保存和加载正常
- [ ] 旧版配置加载后新字段有默认值