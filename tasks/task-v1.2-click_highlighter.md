...
## 前置依赖
- [ ] config.py（mouse_highlight 配置项）

## 子任务

### 1. 信号桥
- [ ] `_ClickBridge(QObject)` 类
  - `click_occurred = pyqtSignal(int, int)` — 鼠标点击 (x, y) 从 pynput 线程转发到 Qt 主线程

### 2. ClickCircle 动画
- [ ] `ClickCircle(QWidget)` 类
...
### 4. 性能考量
- [ ] 每个 ClickCircle 独立 QWidget，动画结束后 deleteLater()
- [ ] pynput.mouse.Listener 独立线程运行
- [ ] 点击回调通过 _ClickBridge 信号桥转发到 Qt 主线程
- [ ] 不在录制中时 start() 不执行（由 main.py 控制）

## 验收标准
- [ ] 左键点击显示扩散圆圈动画，约 300ms 后消失
- [ ] 右键点击不显示动画
- [ ] `mouse_highlight=False` 时 ClickHighlighter 不启动监听
- [ ] 录制停止后动画不再出现
- [ ] 连续快速点击时多个 ClickCircle 同时存在互不干扰
- [ ] ClickCircle 不拦截鼠标事件（设置 WA_TransparentForMouseInput）