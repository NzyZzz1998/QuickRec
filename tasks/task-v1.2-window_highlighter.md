...
### 2. 位置跟踪
- [x] `_timer: QTimer` — 每 100ms 更新位置
- [x] `_update_position()` 方法
...
### 3. 边框绘制
- [x] `paintEvent(event)` 方法
  - 绘制绿色虚线边框：`QPen(QColor(0, 200, 0, 200), 3, Qt.DashLine)`
  - 边框宽度 3px，圆角 0（矩形）
  - 内部完全透明（不遮挡窗口内容）
- [x] `show_highlight()` 方法 — 启动定时器并显示
- [x] `hide_highlight()` 方法 — 停止定时器并隐藏

### 4. 资源清理
- [x] `hide_highlight()` 停止 `_timer`
- [x] Widget 关闭时清理定时器

## 验收标准
- [x] 绿色虚线边框正确包围目标窗口（含边框）
- [x] 窗口移动时边框跟随（100ms 内更新）
- [x] 窗口最小化时边框自动隐藏
- [x] 窗口关闭时边框自动隐藏并停止定时器
- [x] 边框不拦截鼠标事件（点击穿透到下层窗口）
- [x] 边框不遮挡窗口内容（内部透明）