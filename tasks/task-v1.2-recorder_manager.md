...
### 1. RecordMode 枚举扩展
- [ ] 新增 `RecordMode.WINDOW = "window"` 枚举值
- [ ] 新增字段 `_window_hwnd: int | None = None`（目标窗口句柄）
- [ ] 新增字段 `_window_title: str = ""`（目标窗口标题）

### 2. start_window 方法
- [ ] `start_window(self, hwnd: int) -> bool` 方法
...
- [ ] 在录制循环中，当 `_mode == RecordMode.WINDOW` 时：
...
- [ ] 录制过程中窗口移动时捕获区域跟随更新
- [ ] 窗口最小化时发射 `window_lost("minimized")` 信号
- [ ] 窗口关闭时发射 `window_lost("closed")` 信号
- [ ] 全屏录制和区域录制不受影响（回归测试）
- [ ] 窗口录制 + 音频正常工作