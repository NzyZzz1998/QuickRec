...
### 3. 回调字典扩展
- [ ] `_callbacks` 新增 `"start_window": self._on_start_window` 键
- [ ] `_on_start_window()` 方法：通过信号桥发射 `start_window_requested`
...
## 验收标准
- [ ] 空闲状态托盘菜单显示"窗口录制"选项
- [ ] 点击"窗口录制"触发 `start_window_requested` 信号
- [ ] 录制中菜单不变（暂停/继续、停止、设置、打开文件夹、退出）
- [ ] 与已有菜单项功能无冲突