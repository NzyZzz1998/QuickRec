...
- [ ] toolbar.py（倒计时模式）
- [ ] recorder_manager.py（WINDOW 模式 + window_lost 信号）
- [ ] autostart.py（开机自启检查）
- [ ] tray_icon.py（窗口录制菜单项）

## 子任务

### 1. 新增信号桥
- [ ] `_WindowBridge(QObject)` 类
  - `window_selected = pyqtSignal(int, str)` — (hwnd, title)
  - `cancelled = pyqtSignal()`
- [ ] `_WindowLostBridge(QObject)` 类
  - `window_lost = pyqtSignal(str)` — "closed" / "minimized"
- [ ] `_HotkeyBridge` 新增 `window_requested = pyqtSignal()` 信号

### 2. 新增字段
- [ ] `_window_highlighter: WindowHighlighter | None = None`（窗口边框高亮）
- [ ] `_click_highlighter: ClickHighlighter`（鼠标点击高亮）
- [ ] `_window_selector: WindowSelector | None = None`（窗口选择器，防 GC）
- [ ] `_window_bridge: _WindowBridge`（窗口选择信号桥）
...
- [ ] `_on_start_window()` 方法
  ...
  - `exec_()` 显示对话框
- [ ] `_on_window_selected(hwnd, title)` 方法
...
### 5. 鼠标高亮控制
- [ ] `_update_highlight_state()` 方法
  - 判断 `config.mouse_highlight and recorder.state == RECORDING`
  - True → `click_highlighter.start()`
  - False → `click_highlighter.stop()`
- [ ] 录制开始时调用 `_update_highlight_state()`
- [ ] 录制结束时（`_handle_saved`）调用 `_click_highlighter.stop()`

### 6. 窗口高亮生命周期
- [ ] 录制开始时创建 `WindowHighlighter(hwnd)` → `show_highlight()`
- [ ] 录制结束时（`_handle_saved`）调用 `hide_highlight()` + 置 None
- [ ] 录制取消时同理清理
...
### 8. 托盘回调扩展
- [ ] `_callbacks` 字典新增 `"start_window": self._on_start_window`
- [ ] `_hotkey_bridge.window_requested.connect(self._on_start_window)`

### 9. _handle_saved 清理
- [ ] 清理 `_window_highlighter`（hide + None）
- [ ] 停止 `_click_highlighter`

## 验收标准
- [ ] Ctrl+Shift+W 触发窗口选择对话框
- [ ] 托盘菜单"窗口录制"触发窗口选择
- [ ] 窗口选择后正确开始录制 + 边框高亮
- [ ] 窗口关闭/最小化时暂停并弹窗提示
- [ ] 倒计时在所有录制模式（全屏/区域/窗口）下正确工作
- [ ] ESC 取消倒计时正确工作
- [ ] 鼠标高亮在录制开启时启动、停止时关闭
- [ ] 窗口录制结束后清理边框高亮