...
- [ ] `update_region(self, region: tuple) -> None` 方法
...
- [ ] 更新 `get_monitor_size()` 方法
  - 若 `_region` 不为 None，返回 `_region` 的 (width, height)
  - 否则返回全屏尺寸

### 2. 窗口录制兼容
- [ ] 确认 `update_region()` 在录制循环中调用时线程安全
  - `_region` 和 `_dxcam_region` 是简单元组赋值，Python GIL 保证原子性
  - 不需要额外锁

## 验收标准
- [ ] `update_region((100, 200, 800, 600))` 后 `capture_frame()` 返回新区域的帧
- [ ] 区域更新不影响录制帧率（无额外延迟）
- [ ] `get_monitor_size()` 返回当前捕获区域的尺寸
- [ ] 全屏录制时 `update_region` 不影响现有行为