...
FULL_PRODUCT_ID = "QuickRec.Full"
LITE_PRODUCT_ID = "QuickRec.Lite"
...
class WindowsInstanceKernel:
    ...
    def __init__(
        ...
        *,
        ...
        if os.name != "nt":
            raise OSError("QuickRec single-instance protection requires Windows")
...
class SingleInstanceGuard:
    ...
    def is_primary(self) -> bool:
        return (
            self._mutex_handle is not None
            and self._activation_event_handle is not None
        )
    ...
    def acquire(self) -> bool:
        ...
        mutex_handle, already_exists = self._kernel.create_mutex(
            self._mutex_name
        )
        if already_exists:
            try:
                self._activation_signal_sent = (
                    self._kernel.signal_activation_event(
        ...
        try:
            self._activation_event_handle = (
                self._kernel.create_activation_event(
                    self._activation_event_name
                )
            )
        except Exception:
    ...
    def consume_activation_request(self) -> bool:
        if self._activation_event_handle is None:
            return False
        return self._kernel.consume_activation_event(
            self._activation_event_handle
        )
